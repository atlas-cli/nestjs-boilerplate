...
export class BastionStack extends Stack {
    constructor(scope: any, id: string, props: ApplicationProps) {
        ...
        const { vpc, securityGroup } = importGenericSecurityGroupResources(this, scopedName + 'DatabaseSecurityGroup');

        if (props.applications.core) {
            // enable ssh port
            securityGroup.addIngressRule(Peer.anyIpv4(), Port.tcp(22), 'Add public access to ssh');
