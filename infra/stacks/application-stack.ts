...
import { ApplicationConfig, ApplicationProps } from '../props/application-props';
...
export class ApplicationStack extends cdk.Stack {
    constructor(scope: Construct, id: string, applicationProps: ApplicationProps) {
        ...
        const publicHostedZone = route53.PublicHostedZone.fromHostedZoneAttributes(this, 'ApplicationZone', {
            hostedZoneId: applicationConfig.idPublicHostZone,
            zoneName: applicationConfig.domainName,
        });

        const certificate = new certificatemanager.Certificate(this, 'ApplicationCertificate', {
            domainName: applicationConfig.domainName,
            subjectAlternativeNames: [`*.${applicationConfig.domainName}`, applicationConfig.domainName],
            validation: certificatemanager.CertificateValidation.fromDns(publicHostedZone),
        });
        ...
        const { vpc } = importGenericSecurityGroupResources(this, 'CoreDatabaseSecurityGroup');
        ...
        this.createLambdaResources(applicationConfig, applicationProps, api, { sessions: sessionsTable }, [
            { functionName: 'Auth', moduleName: 'auth' },
            { functionName: 'Users', moduleName: 'users' },
            { functionName: 'Swagger', moduleName: 'swagger', swaggerBundling: true },
        ], vpc, lambdaSecurityGroup, lambdaRole);
    ...
    createLambdaResources(
        ...
        resources: { functionName: string; moduleName: string; swaggerBundling?: boolean }[],
        vpc: ec2.IVpc,
        ...
        resources.forEach(({ functionName, moduleName, swaggerBundling }) => {
            ...
            const { nodejsFunction } = new LambdaNestJsFunction(this, `${functionName}Function`, lambdaFunctionProps);

            tables.sessions.grantReadWriteData(nodejsFunction);

            const lambdaIntegration = new apigateway.LambdaIntegration(nodejsFunction, {
                proxy: true,
            });
