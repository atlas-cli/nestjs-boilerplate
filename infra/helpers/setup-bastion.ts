...
import { BastionHostLinux, ISecurityGroup, IVpc, SubnetType } from "aws-cdk-lib/aws-ec2";

export function setupBastion(scope: any, scopedName: string, vpc: IVpc, securityGroup: ISecurityGroup): void {
    // Create bastion host instance in public subnet
    const bastionHostLinux = new BastionHostLinux(scope, `${scopedName}BastionHostLinux`, {
        vpc: vpc,
    ...
    const applicationDatabaseHost = Fn.importValue(`${scopedName}DatabaseClusterHost`);
