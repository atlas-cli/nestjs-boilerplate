...
const bootstrapInfrastructure = (environments: { [key: string]: ApplicationProps }) => {
    const app = new App();

    // Get the current environment from the context or use the default.
    const environment = app.node.tryGetContext('environment') ?? DEFAULT_STAGE_NAME;
    ...
    layersStack.forEach(({ name, provide, dependencies }: any) => {
        ...
        const config = { ...applicationProps, env: applicationProps.env, layersCreated: layerCreated };
 ...
 */
export const bootstrap = (environments: { [key: string]: ApplicationProps }) => {
