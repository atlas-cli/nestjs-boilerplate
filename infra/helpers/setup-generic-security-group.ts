...
const exportResources = (
  ...
  vpc.publicSubnets.forEach((subnet, index) => {
    createOutput(
      scope,
      `${scopedName}GenericSubnetId${index + 1}`,
      subnet.subnetId,
    );
    createOutput(
      scope,
      `${scopedName}GenericSubnetRouteTableId${index + 1}`,
  ...
  vpc.privateSubnets.forEach((subnet, index) => {
    createOutput(
      scope,
      `${scopedName}GenericPrivateSubnetId${index + 1}`,
      subnet.subnetId,
    );
    createOutput(
      scope,
      `${scopedName}GenericPrivateSubnetRouteTableId${index + 1}`,
      subnet.routeTable.routeTableId,
    );
  });
